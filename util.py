...
class SQuAD(data.Dataset):
    ...
    def __init__(self, data_path, is_local=True, use_v2=True):
        super(SQuAD, self).__init__()

        dataset = np.load(data_path)
        num_samples = len(dataset['context_idxs'])
        self.context_idxs = torch.from_numpy(dataset['context_idxs']).long()  # (129922, 400) 400 是 context 的长度
        self.context_char_idxs = torch.from_numpy(dataset['context_char_idxs']).long()  # (129922, 400, 16) 16 是词的长度
        self.question_idxs = torch.from_numpy(dataset['ques_idxs']).long()  # (129922, 50) 50 是问题的长度
        self.question_char_idxs = torch.from_numpy(dataset['ques_char_idxs']).long()  # (129922, 50, 16)
        self.y1s = torch.from_numpy(dataset['y1s']).long()  # (129922,)
        self.y2s = torch.from_numpy(dataset['y2s']).long()  # (129922,)
        self.ids = torch.from_numpy(dataset['ids']).long()  # (129922,)

        if is_local:
            idx = local_idx(num_samples)
            self.context_idxs = self.context_idxs[idx]
            self.context_char_idxs = self.context_char_idxs[idx]
            self.question_idxs = self.question_idxs[idx]
            self.question_char_idxs = self.question_char_idxs[idx]
            self.y1s = self.y1s[idx]
            self.y2s = self.y2s[idx]
        ...
        if use_v2:
            ...
            self.y1s += 1  # 由于在第一个位置添加了 OOV token 所以其他问题的答案开始结束索引都应该加 1
            self.y2s += 1
...
def local_idx(total):
    local_idx = np.random.choice(range(total), int(0.001 * total), replace=False)
    return torch.from_numpy(local_idx).long()


def collate_fn(examples, para_limit, ques_limit, char_limit):
    ...
    def merge_1d(arrays, max_length, dtype=torch.int64, pad_value=0):
        # 按照 batch 进行 pad，之前是整个数据集一个统一的维度，造成空间上的浪费
        lengths = [(a != pad_value).sum() for a in arrays]
        cur_max_length = min(max_length, max(lengths))
        padded = torch.zeros(len(arrays), cur_max_length, dtype=dtype)
        for i, seq in enumerate(arrays):
            end = lengths[i]
            padded[i, :min(end, cur_max_length)] = seq[:min(end, cur_max_length)]
        return padded

    def merge_2d(matrices, max_word_length, max_char_length, dtype=torch.int64, pad_value=0):
        heights = [(m.sum(1) != pad_value).sum() for m in matrices]
        widths = [(m.sum(0) != pad_value).sum() for m in matrices]
        cur_max_h = min(max(heights), max_word_length)
        cur_max_s = min(max(widths), max_char_length)
        padded = torch.zeros(len(matrices), cur_max_h, cur_max_s, dtype=dtype)
        for i, seq in enumerate(matrices):
            height, width = heights[i], widths[i]
            padded[i, :min(height, cur_max_h), :min(width, cur_max_s)] = seq[:min(height, cur_max_h),
                                                                         :min(width, cur_max_s)]
        return padded

    # Group by tensor type
    # 通过 zip 变成 Tuple[tensor]
    context_idxs, context_char_idxs, \
    question_idxs, question_char_idxs, \
    y1s, y2s, ids = zip(*examples)

    # Merge into batch tensors
    context_idxs = merge_1d(context_idxs, para_limit)
    context_char_idxs = merge_2d(context_char_idxs, para_limit, char_limit)
    question_idxs = merge_1d(question_idxs, ques_limit)
    question_char_idxs = merge_2d(question_char_idxs, ques_limit, char_limit)
    y1s = merge_0d(y1s)
    y2s = merge_0d(y2s)
    ids = merge_0d(ids)
...
def discretize(p_start, p_end, max_len=15, no_answer=False):
    ...
    if no_answer:
        ...
        # 第一个位置加上了 OOV token ，用于无答案情况，第一个位置不应该和其他任何非 0 位置组合，所以置为 0
        is_legal_pair[0, :] = 0
        is_legal_pair[:, 0] = 0
    else:
        p_no_answer = None
...
def compute_avna(prediction, ground_truths):
    ...
    只是为了计算有答案-有答案，无答案-无答案的数量，并不在意其正确性，
    只在乎有答案的是否是被预测为有答案的，无答案的被预测为无答案的
    """
