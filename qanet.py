...
class CQAttention(nn.Module):
    ...
    def forward(self, C, Q, Cmask, Qmask):
        ...
        # TODO
        # 补全代码 1
        # 使用下面的提示代码和课上的课件，补全下面 CQAttention 的计算过程
        # https://github.com/heliumsea/QANet-pytorch/blob/master/models.py
        # ----------------------------------------------------------------------------------------

        # S = self.trilinear_for_attention(C, Q)
        # Cmask = Cmask.view(batch_size_c, C.size(1), 1)
        # Qmask = Qmask.view(batch_size_c, 1, Q.size(1))
        return
...
class QANet(nn.Module):
    def __init__(self, word_mat, char_mat):
        super().__init__()
        self.char_emb = nn.Embedding.from_pretrained(char_mat, freeze=args.pretrained_char)
        self.word_emb = nn.Embedding.from_pretrained(word_mat, freeze=True)
        self.emb = Embedding()
    ...
    def forward(self, Cwid, Qwid, Ccid, Qcid):
        ...
        # --------------------------以下为补全代码-----------------------------
        for enc in self.model_enc_blks: M1 = enc(M0, maskC)
        M1 = M0
        for enc in self.model_enc_blks: M1 = enc(M1, maskC)
        M2 = M1
        for enc in self.model_enc_blks: M2 = enc(M2, maskC)
        M3 = M2
        for enc in self.model_enc_blks: M3 = enc(M3, maskC)
